...
\begin{equation}
 \left| 	\frac{df}{dx} \right|_0 = \mu_{m} \left( 1-\frac{2 x}{K}\right)
\end{equation}
...
\paragraph{example 1:} The chemical signaling system in figure \ref has two components $X$ and $R$ which responds to an input signal $S$:
...
Since we are interested in a particular steady state, we substitute its value in the jacobian to obtain the linearized system:
...
And we calculate eigenvalues equating the Jacobian to zero:
...
\paragraph{example 2} A chemostat is a microbial culture fed with fresh medium at a constant rate while the fermentation broth flows out of the culture at identical rate to keep constant volume. This system is described by the equations:
...
The partial derivavives for the Jacobian matrix are:
...
Resulting in the eigenvalues $\lambda_1 = -2 D$ and $\lambda_2 = 2\,(\mu(s_0) - D)$. Remember that the condition for a steady state to be stable is that all their eigenvalues are negative. So the steady state $x=0$,$s=s_0$ is only stable if $D > \mu(s_0)$. In other words, id the dillution rate is higher than the growth rate the cells can achieve at the highest possible substrate concentration, then the cells are washed out $x=0$, and with no cells to consume it, the substrate concentration in the tank will be the same as in the feed.
