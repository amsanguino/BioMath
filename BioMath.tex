...
Once we made a simulation, it is often helpful to show a visual representation. This can be done in many different ways, but we will see the two most common here. On the one hand, we can just plot the results as the state of our system, $x$, against time, as shown in figure \ref{fig:tcourse}.
...
For more complex systems, we may prefer to show the state of the system as point in space. For instance, our first mathematical model has a signle variable so any  state can be represented as a point obn the real line.
...
Biologists often describe growth using its doubling time, $t_d$,  which is the time needed for the system to duplicate ts size:
...
A vector of dimension $n$ is just a collection of $n$ ``normal'' numbers which, by the way, are called scalars. So the members of the total population is vector  $\mathbf{x}$ and it is composed of $x_1$ eggs and $x_2$ animals. We write vectors and matrices (see below) in \textbf{boldface}.
